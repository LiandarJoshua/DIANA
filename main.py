...
def create_event(name, date_time):
    with open('calendar_events.csv', mode='a', newline='') as file:
...
def get_events():
    ...
    try:
        with open('calendar_events.csv', mode='r') as file:
            ...
            for row in reader:
                name, date_time = row
                events.append({'name': name, 'time': date_time})
...
def play_youtube_song(song_name):
    # Use pytube to search for the song
    search = Search(song_name)
    video = search.results[0]  # Get the first result from the search
    video_url = video.watch_url  # Retrieve the video URL
...
def execute_command(query):
    functional_commands = ["open notepad", "open command prompt", 
                           ...
                           "restart", "sleep", "tell me the time", "weather","alarm","wifi speed","internet speed","add task","view tasks","remove task","show tasks","show task","create event","list events"]
    ...
    if any(cmd in query for cmd in functional_commands):
        if "open notepad" in query:
            ...
            song_name = query.replace("play song", "").strip()  # Extract song name from the query
